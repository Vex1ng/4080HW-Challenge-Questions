...
void nativeRuntimeError(const char* format, ...) {
    va_list args;
    va_start(args, format);
    vsnprintf(vm.nativeErrorMsg, NATIVE_ERROR_MAX, format, args);
    va_end(args);
    vm.hasNativeError = true;
}
...
static Value sqrtNative(int argCount, Value* args) {
    if (!IS_NUMBER(args[0])) {
        nativeRuntimeError("sqrt() expects a number argument.");
        return NIL_VAL;
    }
    double n = AS_NUMBER(args[0]);
    if (n < 0) {
        nativeRuntimeError("sqrt() argument must be non-negative.");
        return NIL_VAL;
    }
    ...
    if (IS_OBJ(callee)) {
        switch (OBJ_TYPE(callee)) {
            ...
            case OBJ_NATIVE: {
                ...
                if (vm.hasNativeError) {
                    vm.hasNativeError = false;
                    runtimeError("%s", vm.nativeErrorMsg);
                    return false;
                }
