...
class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
  ...
  private final Map<String, Token> declaredLocals = new HashMap<>();
...
//> end-scope
  private void endScope() {
    ...
    // Report unused locals in the scope we're leaving
    for (String name : declaredLocals.keySet()) {
      if (!usedLocals.contains(name)) {
        Token token = declaredLocals.get(name);
        Lox.error(token, "Local variable '" + name + "' is declared but never used.");
      }
    }

    // Reset tracking for the next scope
...
//< duplicate-variable
    ...
    declaredLocals.put(name.lexeme, name);
  }
//< declare
//> define
