...
//> Statements and State interpret
  void interpret(List<Stmt> statements) {
    interpret(statements, false);  // Default: not REPL (scripts)
  }
...
//> interpret-methods
    ...
    LoxClass metaclass = new LoxClass(
            stmt.name.lexeme + " metaclass", null, staticMethods, null, new HashMap<>());
    ...
    LoxClass klass = new LoxClass(stmt.name.lexeme,
            (LoxClass)superclass, methods, metaclass, getters);
...
//> Inheritance interpreter-visit-super
  ...
  public Object visitSuperExpr(Expr.Super expr) {
    throw new RuntimeError(expr.keyword,
            "'super' is removed. Use 'inner' instead.");
  }
  //< Inheritance interpreter-visit-super
...
//< Resolving and Binding call-look-up-variable
  ...
  public Object visitLambdaExpr(Expr.Lambda expr) {
    Stmt.Function fakeDecl = new Stmt.Function(null, expr.parameters, expr.body);

    return new LoxFunction(fakeDecl, environment, false, false);
  }
  ...
  public Object visitInnerExpr(Expr.Inner expr) {
    ...
    LoxInstance thisInstance = (LoxInstance) environment.getAt(distance, 0);
    LoxClass    ownerClass   = (LoxClass)    environment.getAt(distance, 1);
    String      methodName   = (String)      environment.getAt(distance, 2);
    ...
    LoxClass thisClass = thisInstance.klass;
    LoxClass subclass  = findInnerSubclass(methodName, ownerClass, thisClass);

    if (subclass == null) {
      return null;
    }

    LoxFunction method = subclass.getOwnMethod(methodName);
    LoxFunction bound  = method.bind(thisInstance, subclass);

    if (arguments.size() != bound.arity()) {
      throw new RuntimeError(expr.paren, "Expected " +
              bound.arity() + " arguments but got " + arguments.size() + ".");
    }

    return bound.call(this, arguments);
  }


  private LoxClass nextSubclassOf(LoxClass ancestor, LoxClass descendant) {
    if (descendant == null || descendant == ancestor) return null;
    if (descendant.superclass == ancestor) return descendant;
    return nextSubclassOf(ancestor, descendant.superclass);
  }


  private LoxClass findInnerSubclass(String methodName,
                                     LoxClass ownerClass,
                                     LoxClass thisClass) {
    LoxClass current = nextSubclassOf(ownerClass, thisClass);
    while (current != null) {
      if (current.getOwnMethod(methodName) != null) return current;
      if (current == thisClass) break;
      current = nextSubclassOf(current, thisClass);
    }
    return null;
  }
  ...
  private Object lookUpVariable(Token name, Expr expr) {
    ...
    Object value;
    if (location != null) {
      value = environment.getAt(location[0], location[1]);
    } else {
