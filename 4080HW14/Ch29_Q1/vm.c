        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_SET_PROPERTY: {
                ...
                if (instance->isInitializing) {
                    Value existing;
                    if (tableGet(&instance->fields, OBJ_VAL(name), &existing)) {
                        SAVE_IP();
                        runtimeError("Field '%s' already set by a superclass initializer.", name->chars);
                        return INTERPRET_RUNTIME_ERROR;
                    }
                }
            ...
            case OP_RETURN: {
                ...
                ObjFunction* returning = frame->closure->function;
                vm.frameCount--;
                if (vm.frameCount == 0) {
                    pop();
                    return INTERPRET_OK;
                ...
                if (returning->name != NULL &&
                    returning->name->length == 4 &&
                    memcmp(returning->name->chars, "init", 4) == 0 &&
                    IS_INSTANCE(result)) {
                    AS_INSTANCE(result)->isInitializing = false;
                }
                push(result);
                frame = &vm.frames[vm.frameCount - 1];
                LOAD_IP();
