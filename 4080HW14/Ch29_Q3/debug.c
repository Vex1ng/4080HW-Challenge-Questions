...
// CHANGE 6 - updated byteInstruction to read 2 bytes for the slot index
// before it read 1 byte (uint8_t), now it reads 2 (uint16_t)
// this has to match how emitShort() writes them in compiler.c
static int byteInstruction(const char* name, Chunk* chunk, int offset) {
    uint16_t slot = (uint16_t)((chunk->code[offset + 1] << 8) | chunk->code[offset + 2]);
    printf("%-16s %4d\n", name, slot);
    return offset + 3; // was offset + 2, now +3 because we read 2 bytes not 1
}

static int singleByteInstruction(const char* name, Chunk* chunk, int offset) {
...
int disassembleInstruction(Chunk* chunk, int offset) {
    ...
    switch (instruction) {
        ...
        case OP_CALL:           return singleByteInstruction("OP_CALL",              chunk, offset);
        case OP_CLOSURE:        return closureInstruction("OP_CLOSURE",              chunk, offset);
        case OP_GET_UPVALUE:    return singleByteInstruction("OP_GET_UPVALUE",       chunk, offset);
        case OP_SET_UPVALUE:    return singleByteInstruction("OP_SET_UPVALUE",       chunk, offset);
        ...
        case OP_INNER:          return simpleInstruction("OP_INNER",                 offset);
