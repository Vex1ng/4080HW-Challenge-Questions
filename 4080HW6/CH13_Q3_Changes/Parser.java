...
private Stmt classDeclaration() {
  ...
  while (!check(RIGHT_BRACE) && !isAtEnd()) {
    ...
    if (!isStatic && check(LEFT_BRACE)) {
      // No parameter list — this is a getter.
      consume(LEFT_BRACE, "Expect '{' before getter body.");
      List<Stmt> body = block();
      getters.add(new Stmt.Function(methodName, new ArrayList<>(), body));
    } else {
      // Normal method (or static method) with a parameter list.
      ...
      List<Stmt> body = block();
      Stmt.Function fn = new Stmt.Function(methodName, parameters, body);
      if (isStatic) staticMethods.add(fn);
      else methods.add(fn);
    }
  }

  consume(RIGHT_BRACE, "Expect '}' after class body.");
  return new Stmt.Class(name, superclass, methods, staticMethods, getters);
}
//< Classes parse-class-declaration
...
//< Control Flow or-in-assignment
    ...
    if (match(EQUAL,
      ...
      if (operator.type != EQUAL) {
        TokenType binOp = switch (operator.type) {
        ...
        Token binToken = new Token(binOp, operator.lexeme.substring(0, 1),
                null, operator.line);
        value = new Expr.Binary(expr, binToken, value);
      }
  ...
  private Expr primary() {
    if (match(PLUS, MINUS, STAR, SLASH,
      ...
      if (operator.type == PLUS || operator.type == MINUS) {
        term();
      } else if (operator.type == STAR || operator.type == SLASH) {
        factor();
      } else {
        comparison();
      }
...
//> Inheritance parse-super
    ...
    if (match(SUPER)) {
      throw error(previous(), "'super' is removed. Use 'inner' instead.");
    }
//< Inheritance parse-super
//> Classes parse-this
...
//> Statements and State parse-identifier
    ...
    if (match(INNER)) {
      ...
      return new Expr.Inner(keyword, paren, arguments);
    }
